...
export async function POST(req: NextRequest) {
  try {
    ...
    if (videoMode === "story") {
      characterConfigStr += "\n- **فيلم كرتون كوميدي بحت (لا يبدو كفيديو تعليمي نهائياً!):** المشاهد يتفرج على فيلم كرتون مضحك جداً وطبيعي! الأبطال يعيشون حياتهم اليومية (يلعبون، يتشاجرون، يجرون، أو يقعون في مواقف مضحكة). يُمنع منعاً باتاً أن يبدوا وكأنهم يشرحون درساً أو يستكشفون علمياً. المعلومة يجب أن تظهر في النهاية 'بالصدفة البحتة' داخل نكتة أو موقف عفوي. **يُمنع تماماً توجيه الحديث للمشاهد.**\n" +
      "- **المكان (طبيعي 100% بدون أي لمسة علمية):** المكان يجب أن يكون بيئة كرتونية مذهلة وطبيعية تماماً (مثلاً: غابة ديزني ساحرة، بيت شجرة، أو حديقة جميلة). **يُمنع منعاً باتاً** وجود أي شيء علمي في المكان! (لا تجعلهم داخل جسم الإنسان أبداً، ولا ترسم أعضاء، ولا معامل). صِف البيئة بالإنجليزية في الـ SCENE لتكون طبيعية ومريحة للعين. IMPORTANT: Do NOT include smart screens. The text \"Sci.Rehab Elsibai\" MUST be naturally carved in the background (e.g. on a tree trunk) without the camera focusing on it.\n" +
      "- **القائمة السوداء للكلمات العلمية (قاعدة حياة أو موت):** الذكاء الاصطناعي يخطئ وينسخ أسئلة الدرس الحرفية المكتوبة في الملف ليضعها على لسان الشخصيات! هذا يدمر الفيلم! **يُمنع منعاً باتاً تحت أي ظرف استخدام الكلمات الأكاديمية التالية في الحوار:** (العضو المسؤول، عملية، تبادل الغازات، يا ترى مين، الجهاز، وظيفة، درس). استخدم لغة الشارع والمواقف المضحكة! (مثلاً بدلاً من شرح التنفس علمياً، اجعل دباً سميناً ينهج ويقول للثعلب: 'حرام عليك يا أخي! بالونتين الهوا اللي جوا صدري هيفرقعوا من كتر الجري!'). إياك أن تجعل الحوار يبدو كمعلم يختبر طلابه!\n" +
      "- **البداية المباشرة والمواقف الكوميدية:** الفيديو الأول يبدأ بحدث مضحك فوراً (شجار عفوي، سقوط مضحك، شخصية تنهج بشدة). ادخل في الكوميديا فوراً!\n" +
      "- **الشخصيات (حيوانات غابة عادية جداً):** يُمنع منعاً باتاً تجسيد أعضاء الجسم (ممنوع رئة ناطقة)! اختر 3 شخصيات من حيوانات الغابة العادية (مثل الدب الكسول، الثعلب الماكر، الأرنب السريع). اذكر وصفهم صراحة في الـ CHARACTER بدقة كشخصيات كرتونية لطيفة.\n" +
      "- **اللهجة والحوار (عامية مصرية مضحكة وسريعة):** الحوار يجب أن يكون عبارة عن 'إيفيهات' وشجار كوميدي خفيف بلهجة مصرية شعبية دارجة (مثلاً: \"يا عم سيبني أخد نفسي! إنت بتجري ورايا بقطر؟\"). استخدم الكثير من علامات التعجب (!) لزيادة سرعة وحماس الصوت.\n" +
      "- **الجدار الرابع (ممنوع التحدث للجمهور):** يُمنع منعاً باتاً التحدث للجمهور أو طرح أسئلة عليهم! إياك أن تستخدم كلمات مثل: (يا أصحابي، يا مصريين، يا حلوين، يا أصدقائي، يا جماعة، فكروا معايا). الشخصيات تعيش حياتها المضحكة ولا تعلم بوجود كاميرا!\n" +
...
"]";
    ...
    // Fallback array of models to bypass 503 Overloaded errors on a single model
    ...
    while (retries > 0) {
      try {
        const currentModelName = fallbackModels[5 - retries] || fallbackModels[0];
