...
export async function POST(req: NextRequest) {
  try {
    ...
    if (videoMode === "story") {
      characterConfigStr += "\n- **حبكة اللغز وحل المشكلة (الاستنتاج العلمي):** الأطفال يحبون الألغاز! اجعل القصة عبارة عن (مشكلة صحية أو أزمة) تحدث لإحدى الشخصيات، وباقي الشخصيات يحاولون تشخيص المشكلة واكتشاف حلها. (مثال: حيوان يعاني من مشكلة في التنفس، فيبدأ أصدقاؤه في تتبع مسار الهواء لمعرفة أين المشكلة، ويكتشفون المعلومات العلمية بالاستنتاج!). المشاهد يجب أن يتشوق لمعرفة سبب المشكلة وحلها. **يُمنع تماماً توجيه الحديث للمشاهد.**\n" +
      "- **القائمة السوداء للكلمات المدرسية (قاعدة حياة أو موت):** الذكاء الاصطناعي يخطئ وينسخ أسئلة الدرس الحرفية المكتوبة في الملف ليضعها على لسان الحيوانات! هذا يدمر الفيلم! **يُمنع منعاً باتاً تحت أي ظرف استخدام الكلمات الأكاديمية التالية في الحوار:** (العضو المسؤول، عملية، تبادل الغازات، يا ترى مين، الجهاز، وظيفة). الشخصية المريضة لا تسأل أسئلة علمية! بل تتألم وتشتكي! (مثلاً بدلاً من قول 'ما هو العضو المسؤول عن تبادل الغازات؟'، يجب أن يقول المريض بلهفة: 'أنا بموت! الهوا مش راضي يدخل صدري! الأنبوبة دي مسدودة ليه؟ إلحقوني!'). إياك أن تجعل الحوار يبدو كمعلم يختبر طلابه!\n" +
      "- **المكان (البيئة الداخلية المجسمة):** المكان هو موضوع الدرس ذاته ولكن مجسم كعالم سينمائي مبهر! (مثلاً: كهف وردي عملاق يمثل الرئتين، أو شلال دماء). صِف هذا في الـ SCENE بالإنجليزية. IMPORTANT: Do NOT include smart screens. The text \"Sci.Rehab Elsibai\" MUST be naturally carved in the background without the camera focusing on it.\n" +
      "- **البداية المباشرة (In Media Res):** الفيديو الأول يبدأ وهم في قلب المشكلة! (مثلاً شخصية تتألم أو تختنق، وأصدقاؤها مرعوبون ويحاولون إنقاذها). ادخل في الأكشن والدراما فوراً!\n" +
      "- **الشخصيات (حيوانات الطبيعة):** يُمنع منعاً باتاً تجسيد أعضاء الجسم أو الغازات (ممنوع رئة ناطقة)! اختر 3 شخصيات من حيوانات الطبيعة العادية (مثل الدب، الثعلب، الأرنب، الخ) لتكون هي أبطال القصة الذين يحلون المشكلة. اذكر وصفهم صراحة في الـ CHARACTER بدقة.\n" +
      "- **اللهجة والحوار (عامية مصرية سريعة جداً ودرامية):** الحوار يجب أن يكون سريعاً جداً، وكله حزن، قلق، ومحاولة للإنقاذ (مثلاً: \"يا خبر! إنت مش قادر تتنفس ليه؟ إحنا لازم ننزل نشوف الرئة فيها إيه!\"). استخدم الكثير من علامات التعجب (!) لزيادة سرعة وحماس الصوت.\n" +
      "- **الجدار الرابع (ممنوع التحدث للجمهور):** يُمنع منعاً باتاً التحدث للجمهور أو طرح أسئلة عليهم! إياك أن تستخدم كلمات مثل: (يا أصحابي، يا مصريين، يا حلوين، يا أصدقائي، يا جماعة، فكروا معايا). الشخصيات تعيش في مأزقها ولا تعلم بوجود كاميرا!\n" +
