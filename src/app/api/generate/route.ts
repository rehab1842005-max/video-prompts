...
export async function POST(req: NextRequest) {
  try {
    ...
    if (videoMode === "story") {
      ...
      "- **التأليف السينمائي الخيالي (قاعدة حياة أو موت):** إياك ثم إياك أن تجعل الشخصيات تطرح الأسئلة المكتوبة في الدرس (مثل: ما هو العضو المسؤول؟ كيف نعمل؟). هذا فيلم خيالي! يجب أن تتجاهل النص الحرفي للدرس تماماً، وتستوحي منه فقط \"الفكرة\" لتؤلف بها قصة خيالية 100%. (مثال: إذا كان الدرس عن الرئتين، اجعل القصة عن 'مصنع سحري للرياح في الغابة توقف عن العمل ويحتاج لبالونين كبيرين ليعود للحياة'، فتتحدث الشخصيات عن كيفية إنقاذ الغابة وإصلاح المصنع! لا أحد يذكر كلمة 'عضو' أو 'درس' أبداً!).\n" +
      "- **المكان المتغير مع الأحداث (Dynamic Scene):** المكان يجب أن يعكس الحدث الذي يتحدثون عنه في المشهد بدقة! (إذا كانوا يتحدثون عن الرياح أو التنفس، اجعل الأشجار تتحرك بقوة أو المصنع يعمل. إذا تحدثوا عن الدم، اجعل هناك نهراً أحمر متوهجاً). صِف تفاصيل المكان بالإنجليزية في الـ SCENE لكي تتفاعل البيئة مع القصة ولا تكون ثابتة ومملة! A beautiful 3D Pixar-style cinematic setting. IMPORTANT: Do NOT include any smart screens, whiteboards, or laboratory/studio elements that make it look educational! It must look like a real movie world. However, the exact text \"Sci.Rehab Elsibai\" MUST be naturally integrated into the environment (e.g., carved into a tree, glowing on a rock). The FULL text \"Sci.Rehab Elsibai\" must be perfectly spelled and COMPLETELY VISIBLE. It should quietly exist in the background of EVERY video without drawing attention, and the camera should NEVER focus on it.\n" +
